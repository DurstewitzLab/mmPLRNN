function [z,d,Err,LogLike,Z,U_bar]=StateEstPLRNN_MCs(A,W,C,S,Inp_,mu0_,B,G,h,Bet,X_,C_,z0,d0,options)
...
if isempty(Bet)
    Bet = rand(m,K-1); %init guess
end

clear a b
%% initialize states and constraint vector
n=1; idx=0; k=[];
if ~isempty(z0), z=z0(1:end)'; elseif isempty(z0), z=randn(m*Tsum,1); else z=randn(m*Tsum,1); end;
if ~isempty(d0), d=d0; elseif isempty(d0), d=zeros(1,m*Tsum); d(z>0)=1; else d=zeros(1,m*Tsum); d(z>0)=1; end;
Err=1e16;
...
%% mode search via NR:
    ...
    [ z,r,LogLike,Z,U_bar ] = Semi_NR_V2s(Input, zsv, options);
