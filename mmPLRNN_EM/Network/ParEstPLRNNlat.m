function [mu0,W,A,S,C,h]=ParEstPLRNNlat(EV,S0,C0,Lb,Ub,lam)
if nargin<6, lam=0; end
%
% 
...
if nargin>1 && ~isempty(S0), S=S0;
else
    H=zeros(m);
    for i=1:ntr
        if iscell(EV.Ezz0{i})
            H=H+cell2mat(EV.Ezz0{i})-EV.AllIni0(:,i)*mu0{i}'-mu0{i}*EV.AllIni0(:,i)'+mu0{i}*mu0{i}'+mu0{i}*EV.AllInp(:,i)'*C'+C*EV.AllInp(:,i)*mu0{i}';
        else 
