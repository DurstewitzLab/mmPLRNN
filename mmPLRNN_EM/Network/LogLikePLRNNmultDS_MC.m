function LL=LogLikePLRNNmultDS_MC(A,W,C,S,Inp_,mu0_,B,G,h,Beta,X_,C_,Z_,lam,ChunkList,mod)
...
% 
% log-likelihood for PLRNN system(Multimodal Extension(P.B. 05.19))
...
if iscell(X_), X=X_; Inp=Inp_; mu0=mu0_; Cs=C_;
    ...
    m=length(mu0{1});
    Z=mat2cell(Z_,m,T);
else X{1}=X_; Inp{1}=Inp_; mu0{1}=mu0_; Z{1}=Z_; Cs{1}=C_; [~,T]=size(X_); m=length(mu0{1});end
ntr=length(X);
...
for i=1:ntr   % cycle through all trials
    ...
    LL2=trace(P'*G{k}^-1*P);   % from observation model for that trial
    ll_mc=LL_MC(Beta, Z{i}, T(i), m, Cs{i});
    LL3=2*sum(ll_mc);   % from cat. model for that trial
   
    LL=LL-1/2*(LL0+LL1+LL2+LL3+T(i)*sum(log(diag(G{k})))+T(i)*sum(log(diag(S))));  % works only for diag S,G
end
end


%%
% (c) 2018 Daniel Durstewitz, Dept. Theoretical Neuroscience, Central
...
%%
% 2019 Philine Bommer (Multimodal Extension)
% Institute of Mental Health, Heidelberg University
