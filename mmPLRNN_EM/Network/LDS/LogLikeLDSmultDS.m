function LL=LogLikeLDSmultDS(W,C,S,Inp_,mu0_,B,G,h,X_,Z_,ChunkList)
%
...
for i=1:ntr
    ...
    P=X{i}-B{k}*Z{i}; LL2=trace(P'*G{k}^-1*P);   % from observation model
    %LL=LL-1/2*(LL0+LL1+LL2+T(i)*log(det(G))+T(i)*log(det(S)));
    LL=LL-1/2*(LL0+LL1+LL2+T(i)*sum(log(diag(G{k})))+T(i)*sum(log(diag(S))));
end;
